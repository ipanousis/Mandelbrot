...
cl_program CreateProgram(cl_context context, cl_device_id device, 
  ...
  program = clCreateProgramWithSource(context, 1, (const char**)&srcStr2, NULL, &error);
  if(error){
    fprintf(stderr, "cachedProgram.c::CreateProgram clCreateProgramWithSource failed\n");
    return NULL;
  }
